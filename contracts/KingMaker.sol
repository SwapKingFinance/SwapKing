...
// KingMaker is MasterChef's left hand and kinda a wizard. He can cook up King from pretty much anything!
//
// This contract handles "serving up" rewards for xKing holders by trading tokens collected from fees for King.
...
contract KingMaker {
    ...
    constructor(IUniswapV2Factory _factory, address _bar, address _king, address _weth) public {
        factory = _factory;
        king = _king;
        bar = _bar;
        weth = _weth;
    ...
    function _toWETH(address token) internal returns (uint256) {
        // If the passed token is King, don't convert anything
        if (token == king) {
        ...
        if (token == weth) {
            uint amount = IERC20(token).balanceOf(address(this));
            _safeTransfer(token, factory.getPair(weth, king), amount);
            return amount;
        }
        ...
        pair.swap(amount0Out, amount1Out, factory.getPair(weth, king), new bytes(0));
        return amountOut;
    }
